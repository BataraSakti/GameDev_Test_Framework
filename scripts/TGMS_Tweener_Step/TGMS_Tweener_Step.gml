/// @description  TGMS_Tweener_Step()
/// @function  TGMS_Tweener_Step
function TGMS_Tweener_Step() {
	...
	{     
	    ...
	    {   
	        ...
	        {   
	            ...
	            _index = -1;
				if(qs_exists(delayedTweens, ds_type_list)) {
					if(qs_list_size(delayedTweens) > 0 ) {
						repeat(qs_list_size(delayedTweens))
			            {
			                // Get tween from delayed tweens list
			                var _t = delayedTweens[1][| ++_index];
			                ...
			                else // Continue processing timer
			                {
			                    ...
			                    var _dIndex = -1;
								if(qs_exists(_delays, ds_type_list)){
				                    repeat(qs_list_size(_delays))
				                    {
				                        // Get delay
				                        var _d = _delays[1][| ++_dIndex];
			                ...
			                with(_t[TWEEN.STATE])
			                {
			                    ...
			                    if (_time > 0 && _time < _t[TWEEN.DURATION])
			                    {
			                        ...
			                        else if (is_array(_t[TWEEN.PROPERTY]))
			                        {
			                            ...
			                            {
			                                script_execute(_property[0], script_execute(_t[TWEEN.EASE], _time, _t[TWEEN.START], _t[TWEEN.CHANGE], _t[TWEEN.DURATION]), _property[1]);
			                            }
			                        }
			                    }
			                    else if (_t[TWEEN.TIME_SCALE] != 0)
			                    {
			                        // Update tween based on its play mode
			...
			{   
			    ...
			    if (instance_exists(_t[TWEEN.TARGET]) == false)
			    {
			        ...
			        if (_t[TWEEN.TARGET] == noone)
			        {
			            ...
			            if (_t[TWEEN.EVENTS] != -1 && qs_exists(_t[TWEEN.EVENTS], ds_type_map))
			            {
			            ...
			            else
			            {
			                ...
			                if (_t[TWEEN.EVENTS] != -1)
			                {
			                    ...
			                    if(!is_undefined(_events)){
									if(qs_exists(_events, ds_type_map)) {
